...
def symbolic_reasoning(table_array, verbalized_table, question):
    SYMBOLIC_REASONING_PROMPT_TEMPLATE = '''
## Objective
You are provided with a table, a verbalized table, and a question related to the table.
Your task is to generate Python code that answers the question using the table.
...
'''
    ...
    while True:
        ...
        if retry_count > CODE_MAX_RETRIES:
            ...
            code_result = 'None'
            break
        code = get_openai_llm_response(SYMBOLIC_REASONING_PROMPT_TEMPLATE.format(table=table_md, verbalized_table=verbalized_table, question=question, table_array=table_array), json_output=False)
...
def table_reasoning_for_qa(table_array, verbalized_table, question, task='qa'):
    ...
    if task == 'qa':
        if program_aided:
            textual_reasoning_process, code, code_result = symbolic_reasoning(table_array, verbalized_table, question)
