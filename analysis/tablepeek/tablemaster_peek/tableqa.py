...
sys.path.append('./analysis/tablepeek/tablemaster_peek')
sys.path.append('./')
...
def tablemaster_table_understanding(table_array, question, task='qa', peek_size=10000):
    topheaders, key_column_index, selected_column_indices, ranked_column_indices, sql, selected_row_indices = table_structure_understanding(table_array, question, task=task, peek_size=peek_size)
    verbalized_table, subtable_array, final_selected_column_indices = table_content_understanding(table_array, question, selected_row_indices, selected_column_indices, ranked_column_indices, peek_size)
    final_answer, textual_reasoning_process, symbolic_reasoning_process = table_reasoning_for_qa(table_array, verbalized_table, question, task=task, peek_size=peek_size)
    reasoning_process = {
