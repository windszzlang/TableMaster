...
from baselines.direct_prompt import DIRECT_PROMPT_TEMPLATE
from baselines.cot_prompt import COT_PROMPT_TEMPLATE
from baselines.pot_prompt import POT_PROMPT_TEMPLATE

from opensource_llm.inference import get_local_llm_response


dataset = 'wikitq'
# dataset = 'tabfact'

data_path = f'data/{dataset}/noised_test.jsonl'
...
output_path = f'outputs/analysis/norm/{table_type}/{model}/{prompt_type}/'
...
def worker(i, D):
    ...
    if table_type == 'noised':
        table = D['table']
    elif table_type == 'normalized':
        table = D['original_table']
    question = D['question']
    gt_answer = D['answer']
    ...
    try:
        ...
        if prompt_type == 'direct' or prompt_type == 'cot':
            ...
            predicted_answer = re.sub(r'^.*?answer:', '', str(response).lower().replace('\n', ' ')).strip()
        elif prompt_type == 'pot':
            reasoning_process = response
            code = str(response).replace('```python', '').replace('```', '')
            try:
